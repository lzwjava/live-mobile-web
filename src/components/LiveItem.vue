  ...
  <div class="live-item" @click="goIntro">
    ...
    <div class="live-info">
      ...
      <div class="live-item-tag">
        <span class="time-label" v-if="live.status === 10">
          {{ timeGap }}开始
        </span>

        <span class="on-label" v-if="live.status === 20 || live.status === 25">
          正在直播
        </span>
...
import { timeGap as getTimeGap } from '@/common/util'
...
const timeGap = computed(() => getTimeGap(props.live.planTs))

const goIntro = () => {
  router.push(`/intro/${props.live.liveId}`)
}
</script>
